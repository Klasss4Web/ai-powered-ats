"""
Resume processing, generation, and matching for ATS Matcher Backend
"""
...
def register_resume_routes(app):
    ...
    def process_match():
        ...
        if has_resume_id:
            ...
            resume_filename = f"saved_resume_{resume_id}.pdf"
        else:
    ...
    def generate_cv():
        ...
        try:
            pdf_stream = generate_standard_resume_pdf(original_resume_text)
            return send_file(
                pdf_stream,
    ...
    def process_batch_match():
        ...
        if len(resume_files) > MAX_BATCH_RESUMES:
            return jsonify({"error": f"Maximum {MAX_BATCH_RESUMES} resumes allowed for batch processing."}), 400
    ...
    def save_resume():
        ...
        max_resumes = MAX_SAVED_RESUMES.get(g.user['subscription_type'], 1)
